...
const SpecializationTable = () => {
  ...
  const { specializationsPaginated } = useAppSelector(state => state.specialization);
  const [pageIndex, setPageIndex] = useState(specializationsPaginated!.pageIndex);

  const handleDeletion = (id: number) => {  
    let pIndex = pageIndex;

    if(specializationsPaginated?.items.length == 1) {
        pIndex--;
    }

    axios.delete(`http://localhost:5095/Specialization/id?id=${id}`)
      .then(res => { dispatch(getPaginatedSpecializations(pIndex)); })
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {specializationsPaginated!.items.map(({id, name}, key) => (
              <tr key={key}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <Link to={`/specializations/edit/${id}`} className="hover:text-meta-3">
        ...
        <div className="w-[100%] p-6 flex justify-center items-center" >
          <Pagination action={getPaginatedSpecializations} totalPages={specializationsPaginated!.totalPages} pageIndex={specializationsPaginated!.pageIndex} setPageIndex={setPageIndex} />
        </div>
      </div>
    </div>
  );
