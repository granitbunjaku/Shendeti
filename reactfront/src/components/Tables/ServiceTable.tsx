...
const ServiceTable = () => {
  ...
  const { servicesPaginated } = useAppSelector(state => state.service);
  const [pageIndex, setPageIndex] = useState(servicesPaginated!.pageIndex);

  const handleDeletion = (id: number) => {  
    let pIndex = pageIndex;

    if(servicesPaginated?.items.length == 1) {
        pIndex--;
    }

    axios.delete(`http://localhost:5095/Service/id?id=${id}`)
      .then(res => { dispatch(getPaginatedServices(pageIndex)); })
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {servicesPaginated?.items.map(({id, name, specialization}, key) => (
              <tr key={key}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <Link to={`/services/edit/${id}`} className="hover:text-meta-3">
        ...
        <div className="w-[100%] p-6 flex justify-center items-center" >
          <Pagination action={getPaginatedServices} totalPages={servicesPaginated!.totalPages} pageIndex={servicesPaginated!.pageIndex} setPageIndex={setPageIndex} />
        </div>
      </div>
    </div>
  );
