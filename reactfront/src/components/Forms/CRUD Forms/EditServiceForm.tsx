...
const EditServiceForm = () => {
    ...
    useEffect(() => {
        ...
        axios
        .get(`http://localhost:5095/Service/id?id=${id}`)
        .then(res => {
            setName(res.data.name),
            setSpecializationId(res.data.specialization.id)
        })
    ...
    useEffect(() => {
        if(errors.length == 0) {
          axios.put(`http://localhost:5095/Service/id?id=${id}`, {
              'name': name,
    ...
    const handleEdition = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
