...
import { getAllCities } from '../../slices/citySlice';
import axios from 'axios';
import Loader from '../../common/Loader';
import SelectGroupTwo, { data } from '../../components/Forms/SelectGroup/SelectGroupTwo';
...
const SignUp: React.FC = () => {
  ...
  const [givesBlood, setGivesBlood] = useState('false');
  ...
  useEffect(() => {
    axios.get('http://localhost:5095/User/bloodTypes')
  ...
  useEffect(() => {
    if(errors.length === 0) {
        ...
        if(givesBlood == 'true') {
            data.bloodType = parseInt(bloodType);
        }

        axios.post('http://localhost:5095/User/patient/register', data)
  ...
  const handleLogin = (e: FormEvent) => {
    ...
    if (email === '') {
      missingFields.push('Email field is missing');
    }

    if (username === '') {
      missingFields.push('Username field is missing');
    }

    if (password === '') {
      missingFields.push('Password field is missing');
    }

    if (phoneNumber === '') {
      missingFields.push('Phone number field is missing');
    }

    if (city === '') {
      missingFields.push('City field is missing');
    }

    if (gender === '') {
      missingFields.push('Gender field is missing');
    }
  ...
  const handlePhoneNumberChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const formattedPhoneNumberWithSpaces = formattedPhoneNumber.replace(/(\d{3})(\d{3})(\d{3})/, '$1 $2 $3');
    setPhoneNumber(formattedPhoneNumberWithSpaces);
  };

  return (
    <div className='w-full dark:bg-boxdark bg-white p-4 md:p-6 2xl:p-10 flex flex-col justify-center items-center gap-6'>
    <DarkModeSwitcher />
      <div className="rounded-sm border border-stroke bg-white dark:border-strokedark dark:bg-boxdark w-full">
              ...
              <form onSubmit={handleLogin}>
                ...
                <div className="mb-6">
                  ...
                  <div className="relative">
                    <input
                      type="password"
                      placeholder="6+ Karaktere, 1 Shkronj e madhe"
                      className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                      value={password}
                ...
                <div className="mb-6">
                  <SelectGroupTwo title='A do të jepni gjak?' selectedOption={givesBlood} setSelectedOption={setGivesBlood} data={[{id: true, name: 'Po'}, {id: false, name: 'Jo'}]} />
                </div>
                
                <div className="mb-6">
                  <SelectGroupTwo title='Në cilin qytet jetoni?' selectedOption={city} setSelectedOption={setCity} data={cities} />
                </div>

                <div className="mb-6">
                  <SelectGroupTwo title='Cila është gjinia juaj?' selectedOption={gender} setSelectedOption={setGender} data={[{id: 'Male', name: 'Male'}, {id: 'Female', name: 'Female'}]} />
                </div>

                {
                  givesBlood == 'true' && 
                  <div className="mb-6">
                    <SelectGroupTwo title='Lloji i gjakut?' selectedOption={bloodType} setSelectedOption={setBloodType} data={bloodTypes} />
                  </div>
                }
                ...
                <div className="mt-6 text-center">
                  <p>
                    ...
                    <Link to="/auth/signin" className="text-meta-3">
                      Sign In
                    </Link>
                  </p>
                </div>

                {errors.map(e => <p className="text-red-600 text-center mb-3" key={e}>{e}</p>)}
